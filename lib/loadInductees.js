...
async function loadLiveRows() {
  ...
  return { rows, driveError: getSyncStatus().error, imageErrors: getSyncStatus().imageErrors };
}

async function loadInductees() {
  const live = isLiveModeConfigured();
  const { rows, driveError, imageErrors } = live
    ? await loadLiveRows()
    : { rows: loadLocalRows(), driveError: null, imageErrors: [] };
  ...
  const inductees = rows.map((row) => {
    ...
    return {
      ...
      photoThumbUrl: photo.matched ? photo.thumbUrl : "/assets/icons/blacksmith-silhoutte.png",
  ...
  inductees.imageErrors = imageErrors;
  return inductees;
}
