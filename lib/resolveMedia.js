...
function findMatch(requested, absDir, urlDir, allowedExts) {
  ...
  if (!found) return { matched: false, url: null, requested: requested.trim() };

  // optimizeImages writes a smaller <name>-thumb copy beside each photo; fall
  // back to the full image where there isn't one (hand-placed local files).
  const { name, ext } = path.parse(found);
  const thumb = listDir(absDir).includes(`${name}-thumb${ext}`) ? `${name}-thumb${ext}` : found;
  return {
    matched: true,
    url: `${urlDir}/${found}`,
    thumbUrl: `${urlDir}/${thumb}`,
...
// Untouched Drive originals; optimizeImages turns these into LIVE_MEDIA_DIR.
const DRIVE_DOWNLOAD_DIR = path.join(ROOT, ".cache/drive/inductees");
