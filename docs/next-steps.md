...
- ⬜ **GitHub Actions workflow**: nightly or weekly cron (design doc §5) that pulls the sheet + Drive folder, rebuilds, and deploys to GitHub Pages. Needs the service-account JSON and the sheet/folder IDs as repo secrets (`GOOGLE_SERVICE_ACCOUNT_JSON`, `GOOGLE_SHEETS_ID`, `GOOGLE_DRIVE_FOLDER_ID` — the env vars already exist, this is just wiring them into CI). Next up.
...
- ⬜ **Nomination form URL** for `/nominate/` — the sheet's `params.nomination_form_url` exists but is still blank, so the page is still showing the `TBD` placeholder embed. Once the committee builds that Google Form and drops the URL into that cell, it goes live automatically — no code or deploy needed.
