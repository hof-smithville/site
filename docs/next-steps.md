...
- ⚠️ **Pages needs a public repo or a paid plan.** `sighmon606/shs-hof` is currently private, and GitHub Pages on a private repo requires GitHub Pro/Team. Either make the repo public (check the git history first — see the creds.txt note below), pay for Pro, or host elsewhere (Netlify and Cloudflare Pages both serve private repos on their free tiers).
- ⚠️ **`docs/creds.txt` is still in git history** at commit `e2502bc` with a plaintext password, even though the file was deleted in `8da2004`. It's not publicly readable while the repo is private, but **it becomes readable the moment the repo is made public** — so rotate that password and/or purge the history before flipping visibility.
